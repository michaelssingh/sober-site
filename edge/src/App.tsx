      ...
      <header className="site-header">
        <div className="nav-container">
          <div className="logo-link">
            ...
            <span>Sober<span style={{ color: 'var(--accent-blue)' }}>Edge</span></span>
          </div>
          <nav className="nav-links">
            <a href="https://sober.fyi" className="nav-link">← Back to SOBER</a>
            <button className="cta-btn cta-primary" style={{ padding: '8px 16px', fontSize: '0.85rem' }} onClick={() => setShowFunnel(true)}>
              Get Early Access
            </button>
      ...
      <main style={{ flexGrow: 1 }} className="fade-in">
        ...
        <header className="landing-container" style={{ textAlign: 'center', paddingTop: '80px', paddingBottom: '60px' }}>
          ...
          <div className="hero-ctas" style={{ justifyContent: 'center' }}>
            <button className="cta-btn cta-primary" style={{ padding: '16px 32px', fontSize: '1.1rem' }} onClick={() => setShowFunnel(true)}>
          ...
          <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            <div className="terminal-window">
              <div className="terminal-header">
                ...
                <span className="terminal-title">sober@edge: ~/demo</span>
              </div>
              <div className="terminal-body" style={{ minHeight: '320px' }}>
                <div className="space-y-2">
                  ...
                  {isRunning && <div style={{ display: 'inline-block', width: '2px', height: '15px', background: 'var(--accent-blue)', animation: 'blink 0.8s infinite' }}></div>}
                </div>
                {!isRunning && lines.length === 0 && (
                  <div style={{ display: 'flex', justifyContent: 'center', marginTop: '60px' }}>
                    <button className="terminal-btn" onClick={() => {setLines([]); setStepIndex(0); setIsRunning(true)}}>
              ...
              <div style={{ padding: '32px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                  <div style={{ flex: 1, height: '4px', background: 'var(--border)', borderRadius: '2px', marginRight: '16px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', background: 'var(--accent-blue)', width: `${(funnelStep / 4) * 100}%`, transition: 'all 0.5s ease' }}></div>
                  </div>
                ...
                {funnelStep === 3 && (
                  <div className="fade-in">
                    ...
                    <form style={{ display: 'flex', flexDirection: 'column', gap: '16px' }} onSubmit={(e) => { e.preventDefault(); setFunnelStep(4) }}>
                      ...
                      <button type="submit" className="cta-btn cta-primary" style={{ width: '100%', justifyContent: 'center', padding: '16px' }}>Get My Performance Audit</button>
                    </form>
                  </div>
                )}
                ...
                {funnelStep === 4 && (
                  <div style={{ textAlign: 'center' }} className="fade-in">
                    ...
                    <button className="cta-btn cta-primary" style={{ width: '100%', justifyContent: 'center', padding: '16px' }} onClick={() => setShowFunnel(false)}>Book Implementation Call</button>
                  </div>
                )}
              </div>
            </div>
